/**
 * TeacherLayout - Sidebar + Header + Content for GiangVien
...
function getSelectedKey(pathname) {
  if (pathname === '/teacher' || pathname === '/teacher/') return '/teacher';
  const match = menuItems.find((item) => pathname.startsWith(item.key) && item.key !== '/teacher');
...
export default function TeacherLayout() {
  ...
  const userMenuItems = [
    ...
    },
    {
      ...
      icon: <LockOutlined />,
      label: 'Đổi mật khẩu',
      onClick: () => navigate('/teacher/profile?action=change-password'),
    },
    {
      type: 'divider',
    },
  ...
  const userCode = user?.maGV || getUserCode(user);
          ...
          {!collapsed ? (
            <div className={styles.logoExpanded}>
              ...
              <div className={styles.logoText}>
                ...
                <span className={styles.logoSubtitle}>Giảng viên</span>
              </div>
            </div>
          ) : (
