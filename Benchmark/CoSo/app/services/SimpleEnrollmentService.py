...
def get_site_prefix(identifier: str) -> str:
    prefixes = ["HD", "HL", "NT"]
    for p in prefixes:
        if p in identifier.upper() or identifier.upper().startswith(p):
            return p
    return "HD"
...
class SimpleEnrollmentService:
    ...
    def register_simple(user_id: str, ma_lop_hp: str):
        ...
        try:
            ...
            with open_db_by_branch(home_site) as db_home:
                ...
                for en in existing:
                    if en.MaHP == new_class.MaHP and en.MaHocKy == new_class.MaHocKy:
                        # Đã đăng ký cùng môn → Đây là yêu cầu ĐỔI LỚP
                        old_enrollment = en
                        old_site = home_site
                        break

                if not old_enrollment:
                    # Tìm trong cross-transfer
            ...
            if old_enrollment:
                ...
                try:
                    ...
                    if cross_transfers:
                        ...
                        with ThreadPoolExecutor(max_workers=min(len(cross_transfers), 4)) as ex:
                    ...
                    CONFLICT_CHECK_DURATION.labels(app="coso").observe(check_duration)
            ...
            if "deadlock" in err_msg:
                DB_ERRORS_TOTAL.labels(type="deadlock", app="coso").inc()
            elif "lock timeout" in err_msg or "timeout" in err_msg:
                DB_ERRORS_TOTAL.labels(type="timeout", app="coso").inc()
            else:
                DB_ERRORS_TOTAL.labels(type="db_other", app="coso").inc()
        ...
        if home_site == target_site:
            with open_db_by_branch(home_site) as db:
                ...
                try:
                    ...
                    sec.SiSoHienTai += 1
                    db.commit()
                    LOCK_HOLD_DURATION.labels(type="enroll_local", app="coso").observe(time.perf_counter() - start_lock)
                    return {"status": "Success", "type": "new", "message": f"Đăng ký cục bộ ({home_site})"}
                except Exception as e:
                    db.rollback()
                    LOCK_HOLD_DURATION.labels(type="enroll_local_fail", app="coso").observe(time.perf_counter() - start_lock)
                    raise e
        else:
            ...
            try:
                ...
                db_t.commit()
                db_h.commit()
                LOCK_HOLD_DURATION.labels(type="enroll_cross", app="coso").observe(time.perf_counter() - start_lock)
                return {"status": "Success", "type": "new", "message": f"Đăng ký chéo ({home_site} -> {target_site})"}
            except HTTPException as e:
                db_t.rollback()
                db_h.rollback()
                LOCK_HOLD_DURATION.labels(type="enroll_cross_fail", app="coso").observe(time.perf_counter() - start_lock)
                raise e
            except Exception as e:
                db_t.rollback()
                db_h.rollback()
    ...
    def _do_swap(user_id, home_site, old_ma_lop_hp, old_site, new_ma_lop_hp, new_site, new_class):
        """Thực hiện Cancel lớp cũ + Enroll lớp mới trong 1 transaction (best-effort 2-phase)."""
        ...
        try:
            ...
            # 3. Cập nhật bảng transfer ở home nếu cần
            ...
            LOCK_HOLD_DURATION.labels(type="swap", app="coso").observe(time.perf_counter() - start_lock)
            return {"status": "Success", "type": "swap", "message": f"Đổi lớp {old_ma_lop_hp} → {new_ma_lop_hp}"}
        except HTTPException as e:
            for db in dbs.values():
                db.rollback()
            LOCK_HOLD_DURATION.labels(type="swap_fail", app="coso").observe(time.perf_counter() - start_lock)
            raise e
        except Exception as e:
            for db in dbs.values():
                db.rollback()
            ...
            if home_site == target_site:
                with open_db_by_branch(home_site) as db:
                    ...
                    try:
                        ...
                        db.commit()
                        LOCK_HOLD_DURATION.labels(type="cancel_local", app="coso").observe(time.perf_counter() - start_lock)
                        return {"status": "Success", "message": f"Hủy cục bộ ({home_site})"}
                    except Exception as e:
                        db.rollback()
                        LOCK_HOLD_DURATION.labels(type="cancel_local_fail", app="coso").observe(time.perf_counter() - start_lock)
                        raise e
            else:
                db_t = open_db_by_branch(target_site)
                db_h = open_db_by_branch(home_site)
                ...
                try:
                    ...
                    db_t.commit()
                    db_h.commit()
                    LOCK_HOLD_DURATION.labels(type="cancel_cross", app="coso").observe(time.perf_counter() - start_lock)
                    return {"status": "Success", "message": f"Hủy chéo ({home_site} -> {target_site})"}
                except HTTPException as e:
                    db_t.rollback()
                    db_h.rollback()
                    LOCK_HOLD_DURATION.labels(type="cancel_cross_fail", app="coso").observe(time.perf_counter() - start_lock)
                    raise e
                except Exception as e:
                    db_t.rollback()
                    db_h.rollback()
