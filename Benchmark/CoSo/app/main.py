...
app = FastAPI(title="Benchmark CoSo API")
...
async def collect_metrics(request: Request, call_next):
    ...
    try:
        ...
        HTTP_REQUESTS_TOTAL.labels(method=method, path=path, status=status_code, app="coso").inc()
        HTTP_REQUEST_DURATION.labels(method=method, path=path, app="coso").observe(latency)
